...
// 3. Mutação para Salvar Nível VIP
  const mutacaoSalvarVip = useMutation({
      mutationFn: async (payload) => await api.post('/admin/customers/vip-levels', payload),
        // MUDE PARA:
        onSuccess: () => {
  ...
  const salvarVIPReal = () => {
      ...
      // Traduz os nomes para o Laravel (snake_case) e limpa as vírgulas dos números!
      const payload = {
...
// 5. Ações de Tags (Conectadas ao Banco)
 ...
 // ==========================================
  // BUSCA DE DADOS INTELIGENTE COM REACT QUERY
  // ==========================================
  const { data: listaClientesDaApi = [], isLoading: carregandoClientes } = useQuery({
      queryKey: ['clientesCRM'], 
      queryFn: async () => {
          // CHAMA O BANCO DE DADOS REAL DO LARAVEL COM SUPERPODERES
